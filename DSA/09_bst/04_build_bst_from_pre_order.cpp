...
public:
    ...
    void buildFromPreOrder(vector<int> arr);
    void buildFromPreOrderRec(vector<int> arr);
    void buildFromPostOrder(vector<int> arr);
...
void BinTree ::buildFromPreOrderRec(vector<int> arr)
{
    // works on the principal that har element apni range me legega

    // for root node range can be from -oo to +oo
    int mini = INT_MIN;
    int maxi = INT_MAX;
